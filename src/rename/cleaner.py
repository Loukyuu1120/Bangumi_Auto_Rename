...
def match_and_extract(input_string: str) -> Optional[Tuple[int, int]]:
    ...
    # 2. 中文 第x集 格式
    cn_pattern = re.search(
        r"第\s*(\d+|[零一二三四五六七八九十百千万]+)\s*[集话]", input_string
    )
    if cn_pattern:
    ...
    isolated_num_match = re.search(
        r"(?:^|[^a-zA-Z0-9])(\d{1,3})(?:[^a-zA-Z0-9]|$)", input_string
    )
...
def parse_filename(filename: str) -> Tuple[str, int, Optional[int], Optional[int]]:
    ...
    # Pattern B: S01-S02
    s_range_match = re.search(
        r"[.\s\-_]S(\d{1,2})\s*-\s*S?(\d{1,2})", name, re.IGNORECASE
    )
    # Pattern C: S01
    s_only_match = re.search(r"[.\s\-_]S(\d{1,2})(?:$|[.\s\-_])", name, re.IGNORECASE)

    # Pattern Season X
    season_word_match = re.search(
        r"[.\s\-_]Season[.\s\-_]*(\d{1,2})", name, re.IGNORECASE
    )

    # Pattern 第X季
    cn_season_match = re.search(
        r"[.\s\-_]?第\s*(\d+|[零一二三四五六七八九十百千万]+)\s*季", name
    )

    matches = []
    if se_match:
        matches.append((se_match.start(), se_match, "se"))
    if s_range_match:
        matches.append((s_range_match.start(), s_range_match, "range"))
    if s_only_match:
        matches.append((s_only_match.start(), s_only_match, "s_only"))
    if season_word_match:
        matches.append((season_word_match.start(), season_word_match, "season_word"))
    if cn_season_match:
        matches.append((cn_season_match.start(), cn_season_match, "cn_season"))
    ...
    if year == 0 and season_num is None:
        res_match = re.search(
            r"[.\s\-_](1080[PpIi]|4[Kk]|2160[Pp]|720[Pp])", title_part, re.IGNORECASE
        )
        if res_match:
            title_part = title_part[: res_match.start()]
        else:
            codec_match = re.search(
                r"[.\s\-_]([xXhH]\.?26[45]|HEVC|AVC)", title_part, re.IGNORECASE
            )
            if codec_match:
...
def extract_media_info(filename: str) -> Dict[str, str]:
    ...
    # 提取分辨率
    res_match = re.search(
        r"(2160[Pp]|4[Kk]|1080[PpIi]|720[Pp]|480[Pp]|576[Pp])", filename
    )
    if res_match:
    ...
    group_match = re.search(
        r"-([a-zA-Z0-9_]+)(?:\[.*?\])?(?:\.[a-zA-Z0-9]{2,4})?$", filename
    )
    if group_match:
...
def get_render_context(
    path: Path, info: Dict, season: int = None, episode: int = None
) -> Dict:
    # 1. 首先从文件名提取技术参数
    ...
    if (
        ...
        and is_season_name(path.parent.name)
        and path.parent.parent != path.root
    ):
